...
(docs/auto-attack-rewind-design-2026-07-04.md §4):
  ...
  config    — fresh DB: `auto_swing_enabled` ships default ON (absent row).
...
Two measurement builds isolate the moving reach fixture from the real-swing
defense rider (compile-time flags in server/src/npcs.rs):

  # standing signal + rewind geometry (run focused timing legs independently)
  ARENA_DATABASE=s9probe ARENA_PROJECTILE_LOAD_HARNESS=1 \
      ARENA_NPC_NO_ATTACK=1 ARENA_NPC_TANKY=1 ARENA_NPC_AGGRO_RADIUS=100 \
      ARENA_GENERATE_BINDINGS=0 ARENA_VERIFY_DOTNET=0 \
      ./ops/republish-local-clear.sh
  python3 ops/s9-auto-rewind-probe.py --database s9probe --leg off
  python3 ops/s9-auto-rewind-probe.py --database s9probe --leg on

  # `--leg rewind` remains a combined config/standing/OFF/ON/staleness
  # convenience run, but its two timing-sensitive flip legs are more reliable
  # as independent processes and are the acceptance recipe above.

  # defense-late telemetry rider
  ARENA_DATABASE=s9probe ARENA_PROJECTILE_LOAD_HARNESS=1 \
      ARENA_NPC_HARMLESS=1 ARENA_NPC_TANKY=1 ARENA_NPC_AGGRO_RADIUS=100 \
      ARENA_GENERATE_BINDINGS=0 ARENA_VERIFY_DOTNET=0 \
      ./ops/republish-local-clear.sh
  python3 ops/s9-auto-rewind-probe.py --database s9probe --leg rider
    ...
    if state is None or state[0] != kobold_ref["hex"]:
        ...
        for attempt in range(3):
            ...
            result, reason = attacker.press_melee_at(
                ARM_STRIKE, kobold_ref["hex"], token, 0, ax, ay, az, yaw
            )
            state = auto_state(attacker, attacker.identity)
...
def flip_leg(
    ...
    for index in range(cycles):
        ...
        print(
            ...
            f"max_d={outcome['max_distance']:.2f}m "
...
def main():
    ...
    parser.add_argument(
        "--leg",
        choices=["rewind", "off", "on", "rider"],
        default="rewind",
        help="'off'/'on' iterate one flip leg; 'rider' runs defense telemetry only",
    )
    ...
    configure_probe_combat_build(attacker, [ARM_ABILITY])

    if args.leg == "rewind":
        print("\n== config: auto_swing_enabled ships default ON")
        rows = attacker.sql(
            "SELECT config_id, enabled, max_rewind_ms, auto_swing_enabled FROM combat_lag_comp_config"
        )
        default_on = not rows or rows[0][3].lower() == "true"
        expect(
            "auto_swing default",
            default_on,
            f"rows={rows} (absent row = master ON / auto_swing ON)",
            failures,
        )
    ...
    reach_rows = attacker.sql(
        "SELECT combat_discipline_id, action_id, range FROM auto_attack_catalog"
    )
    strike_rows = attacker.sql("SELECT owner, strike_id FROM auto_attack_state")
    active_rows = attacker.sql(
    ...
    auto_range = next(
        (
            float(rng)
            for discipline, action_id, rng in reach_rows
            if discipline == active_discipline and action_id == armed_strike
        ),
        2.5,
    )
    ...
    print("\n== flip leg, auto_swing OFF: due swings must hold present-time")
    ...
    set_lag_comp(attacker, True, True)  # leave the shipped switch state behind
    finish(failures)
