...
Typical isolated local run:
  ...
  ARENA_DATABASE=lightningreflexesprobe ARENA_PROJECTILE_LOAD_HARNESS=1 \
    ARENA_GENERATE_BINDINGS=0 ARENA_VERIFY_DOTNET=0 \
    ./ops/republish-local-clear.sh
  python3 ops/lightning-reflexes-probe.py --database lightningreflexesprobe
...
LIGHTNING_REFLEXES_ABILITY = "DAGGER_LIGHTNING_REFLEXES"
LIGHTNING_REFLEXES = "LIGHTNING_REFLEXES"
SMITE_ABILITY = "SPELL_SMITE"
SMITE = "SMITE"
FROST_NOVA_ABILITY = "SPELL_FROST_NOVA"
FROST_NOVA = "FROST_NOVA"
NECROTIC_AURA_ABILITY = "SPELL_NECROTIC_AURA"
NECROTIC_AURA = "NECROTIC_AURA"
BLINDING_LIGHT_ABILITY = "SPELL_BLINDING_LIGHT"
...
class Probe:
    ...
    def activate_discipline(self, combat_discipline_id):
        ...
        wait_until(
            f"{self.name} activates {combat_discipline_id}",
            lambda: next(
                (
                    True
                    for owner, discipline in self.sql(
                        "SELECT owner, combat_discipline_id "
                        "FROM active_combat_build_discipline"
                    )
                    if normalize_identity(owner) == self.identity
                    and str(discipline) == combat_discipline_id
                ),
                False,
            ),
        )
...
def main():
    ...
    try:
        ...
        configure_probe_combat_build(
            rogue,
            [LIGHTNING_REFLEXES_ABILITY, BLINDING_LIGHT_ABILITY],
        ...
        configure_probe_combat_build(
            attacker,
            [SMITE_ABILITY, FROST_NOVA_ABILITY, NECROTIC_AURA_ABILITY],
        )
        ...
        time.sleep(2.0)
        rogue.activate_discipline("STAFF")
        ...
        rogue.activate_discipline("DAGGERS")
