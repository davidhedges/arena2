...
victim's current armor/jewelry. All displacement, resistance, interruption,
combat-entry, and teardown behavior runs through the production paths. The
former implicit-deck-boundary assertion is intentionally absent: commit
`615d3110` removed that invisible boundary, and the current arena collision
export contains no replacement blocker.

Typical isolated local run:

  ARENA_DATABASE=knockbackprobe ARENA_PROJECTILE_LOAD_HARNESS=1 \
...
SHOCKWAVE_ABILITY = "WARRIOR_SHOCKWAVE"
SHOCKWAVE = "SHOCKWAVE"
...
def main():
    ...
    try:
        ...
        configure_probe_combat_build(attacker, [SHOCKWAVE_ABILITY])
