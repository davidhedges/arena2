...
Run this against a dedicated local database built from the current module:
  ...
  ARENA_NPC_AI_DEBUG=1 spacetime publish --delete-data=always --yes \
    -s local -p server npcsupportprobe
  spacetime call -s local npcsupportprobe publish_spell_definitions
  spacetime call -s local npcsupportprobe publish_progression_catalogs
  python3 ops/npc-support-decision-probe.py
...
def main():
    ...
    try:
        ...
        probe.call("spawn_npc", [KOBOLD_TEMPLATE, KOBOLD_TEMPLATE, "HOSTILE"])
        time.sleep(0.5)
