...
The vehicle is SPELL_ICE_SPIKES: a no-target SPELL AREA (CASTER_CONE) sweep, range
...
reach through ordinary player authorization). A feature-gated local setup call
validates and freezes that exact canonical assignment before the probe begins;
learned-spell authorization is deliberately not involved.
...
SWEEP_ABILITY = "SPELL_ICE_SPIKES"
SWEEP_SPELL = "ICE_SPIKES"  # no-target SPELL AREA CASTER_CONE, range 7.5 m, 30 mana
    ...
    while len(results) < presses_wanted and time.time() < deadline:
        ...
        if in_flip and now - last_press_at > PRESS_GAP_S:
            ...
            result, reason = attacker.press_cast_at(
                SWEEP_SPELL, token, view_ms, ax, ay, az, yaw
            )
...
def degradation_press(attacker, kobold_ref, database, caster_short):
    ...
    result, reason = attacker.press_melee_at(
        SWEEP_SPELL, "", token, DEGRADE_VIEW_MS, ax, ay, az, ayaw
    )
...
def main():
    ...
    configure_probe_combat_build(attacker, [SWEEP_ABILITY])

    # --- Check 1a: config default ON on a fresh DB (before any set call) ------
    print("\n== config: sweep_rewind_enabled ships default ON")
    rows = attacker.sql(
        "SELECT config_id, enabled, max_rewind_ms, auto_swing_enabled, sweep_rewind_enabled "
        "FROM combat_lag_comp_config"
    )
    default_on = not rows or rows[0][4].lower() == "true"
    expect(
        "config default sweep_rewind ON",
        default_on,
        f"rows={rows} (absent row = shipped default: sweep rewind ON)",
        failures,
    )
    ...
    if os.environ.get("S10_DIAG"):
        ...
        try:
            ...
            attacker.press_cast_at(
                SWEEP_SPELL, "diagcast", int(now_server_ms - VIEW_DELAY_MS), ax, ay, az, ayaw
            )
            time.sleep(1.0)
    ...
    # `rewound_ms` is the age of the selected sparse history sample, not the
    # requested view delay. Unchanged poses are deliberately not resampled,
    # so a valid History sample can be older than the separately verified
    # 250 ms request cap.
    rewind_ok = bool(used_rewinds) and all(r >= 150 for r in used_rewinds)
    source_ok = used_sources.get("history", 0) >= 1
    expect(
        "history-backed nonzero rewind",
        rewind_ok and source_ok,
        f"used-flip sample ages={sorted(used_rewinds)} ms (expect >=150); "
