...
(docs/lag-compensation-design-2026-07-04.md §4):
  ...
  sanity   — config row defaults (disabled, 250 ms cap); an honest small
...
damage:
  ...
  spacetime publish --delete-data=always --yes \
      --bin-path server/target/wasm32-unknown-unknown/release/arena.wasm s8probe
...
def set_lag_comp(probe, enabled):
    probe.call("set_lag_comp_config", [enabled, 250])
    time.sleep(0.6)
...
def main():
    ...
    attacker.call("assign_character_action_bar_ability_to_slot", ["SLOT_1_1", CHARGE_ABILITY])
    time.sleep(0.6)
    ...
    fresh_default_off = not rows or rows[0][1].lower() == "false"
    if not fresh_default_off:
        # A reused DB keeps the last run's toggle; only a fresh publish
        # witnesses the shipped default. Reset and continue.
        print(f"  [NOTE] config row carried over from a prior run: {rows} — resetting OFF")
    ...
    result, reason = attacker.press_melee(MELEE_ABILITY, dummy, "s8-sanity-off", 0)
    ...
    result, reason = attacker.press_melee(MELEE_ABILITY, dummy, "s8-sanity-on", wall_ms)
    expect(
