...
namespace Arena.UI
{
    ...
    public static class ActionBarDropApplier
    {
        public static void ApplyDrop(DbConnection conn, ActionBarDragPayload payload, string? targetSlotId)
        {
            if (!payload.HasValue)
                return;
            ...
            if (string.IsNullOrWhiteSpace(normalizedTargetSlotId))
            {
                if (payload.HasSourceSlot)
                    conn.Reducers.ClearCharacterActionBarSlot(payload.SourceSlotId);
                return;
            }
            ...
            CharacterActionBarAssignment? targetActionBarAssignment = FindCharacterActionBarAssignment(conn, normalizedTargetSlotId);
            if (payload.HasSourceSlot && targetActionBarAssignment != null)
            {
                AssignExistingActionBarAssignmentToSlot(conn, targetActionBarAssignment, payload.SourceSlotId);
                AssignPayloadToSlot(conn, payload, normalizedTargetSlotId);
                return;
            }
            if (payload.HasSourceSlot && targetActionBarAssignment == null && PayloadIsSpell(conn, payload))
            {
                conn.Reducers.ClearCharacterActionBarSlot(payload.SourceSlotId);
                AssignPayloadToSlot(conn, payload, normalizedTargetSlotId);
                return;
        ...
        private static CharacterActionBarAssignment? FindCharacterActionBarAssignment(DbConnection conn, string slotId)
        {
            if (!conn.Identity.HasValue)
                return null;

            string combatProfile = CombatProfileResolver.ResolveForOwner(conn, conn.Identity.Value);
            foreach (CharacterActionBarAssignment assignment in conn.Db.CharacterActionBarAssignment.Owner.Filter(conn.Identity.Value))
            {
        ...
        private static void AssignExistingActionBarAssignmentToSlot(
            DbConnection conn,
            CharacterActionBarAssignment assignment,
            string targetSlotId)
        {
            ...
            {
                conn.Reducers.AssignCharacterActionBarAbilityToSlot(targetSlotId, actionId);
                return;
            }

            conn.Reducers.AssignCharacterActionBarSlot(targetSlotId, actionKind, actionId);
        }
            ...
            string targetSlotId)
        {
            ...
            {
                conn.Reducers.AssignCharacterActionBarAbilityToSlot(targetSlotId, payload.ActionId);
                return;
            }

            conn.Reducers.AssignCharacterActionBarSlot(targetSlotId, payload.ActionKind, payload.ActionId);
        }
