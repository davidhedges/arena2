...
- Normal authoritative player movement now uses swept entry checks for legacy primitive collision and author mesh-collider movement segments. Server movement and Unity local prediction both build deterministic triangle-level movement hulls from exported author mesh data, query them through static broadphase grids, and avoid full-scanning all scene movement hulls during normal movement.
- Authored movement mesh triangles now also contribute to ground-height sampling on both server and client. The sampler queries nearby movement mesh hulls through the same broadphase, rejects steep or vertical triangles, computes the triangle-plane height at `(x,z)`, and only accepts surfaces within the existing snap-up ceiling.
- Movement broadphase now has focused regression coverage for both primitive movement boxes and author mesh movement hulls. The tests verify that small movement sweeps use broadphase candidates and preserve the same blocker that full-scan narrowphase would find.
- Movement candidate-count instrumentation is in place on the server and Unity prediction path. Both sides periodically log box and mesh movement broadphase query counts, average candidates, fallback counts, fallback ratio, last candidate count, and total collider/hull count.
- Projectile/LOS query mesh hits now account for projectile radius against author mesh triangles. The mesh path uses the existing top-level instance broadphase and per-geometry BVH, expands BVH node tests conservatively, then resolves exact world-space swept sphere contact against triangle faces, edges, and vertices.
...
- Legacy primitive collision is now treated as migration compatibility only. Generated movement hull assets/tooling are absent, and `3c Prepare Selected Variant Collision Roles` replaces older direct Arena primitive collision children with author mesh movement/query children when author mesh sources exist.

Immediate next steps:

- Rebuild/restart the server and let Unity recompile, then run a focused `Giant_Skeleton` walking pass over skull, ribcage, neck, slabs, cliffs, and stair/ledge-like surfaces. Watch for movement blocker logs, large correction logs, grounding jitter, and cases where projectile/query collision disagrees with movement collision.
- Review movement candidate-count logs during normal player movement in `Giant_Skeleton`. The expected shape after broadphase is small local candidate sets per substep rather than tens of thousands of triangle hull checks.
- If a specific author mesh collider still causes broad or noisy movement behavior, segment that author mesh deterministically. Do not replace it with primitive approximations.
- Once `Giant_Skeleton` is stable, repeat the same migration and validation loop for the next open-world scene set before broad adoption.
...
- Current mesh validation requires static non-trigger non-convex mesh colliders, readable mesh assets with stable AssetDatabase GUIDs, valid triangle indices, finite vertices/transforms, degenerate-triangle removal, a per-geometry budget of 4,096 triangles, and a per-scene budget of 200,000 unique mesh-geometry triangles.
...
- Projectile/LOS raycasts now query a top-level broadphase over mesh instances, transform candidate rays into mesh-local space, test triangles with projectile radius, and merge the nearest mesh hit with heightfield and generated-open-world hits.
- Projectile tick metrics track mesh broadphase candidates, triangle tests, and mesh broadphase full-scan fallbacks.
- Mesh narrowphase now handles both exact ray-vs-triangle and swept sphere-vs-triangle. Projectile radius is tested against triangle faces, edges, and vertices so mesh-edge grazing is covered.
...
- The projectile load harness can now expose mesh-query work through `world_query_mesh_broadphase_candidates`, `world_query_mesh_bvh_node_tests`, `world_query_mesh_triangles_tested`, and `world_query_mesh_full_scan_fallbacks`.
- A representative harness run near an authored query mesh reported `1,046` total mesh broadphase candidates, `1,678` total BVH node tests, `140` total triangle tests, and `0` mesh full-scan fallbacks. That validates the active mesh path, but it is not a final performance benchmark.
