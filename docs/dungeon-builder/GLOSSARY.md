...
| **Eligible role / eligible beat** | Exact compatibility filters on a recipe. A recipe can fill an opportunity only when both the node's role and beat occur in its eligible lists. Eligibility means “may be selected here”; it does not guarantee selection and does not confer gameplay behavior. |
...
| **Recipe opportunity** | A generated degree-two route-node binding through which zero or more compatible enabled recipes may enter a dungeon. The family constrains the opportunity count; it does not author fixed slot names or exact nodes. Selection resolves before embedding, while an unselected opportunity or one with no compatible active candidate uses generated construction. |
