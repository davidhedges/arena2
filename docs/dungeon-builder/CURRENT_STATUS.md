...
- The Phase 7 automated sweep gate now passes. At this evidence point, curated-gallery review and collision-export-parity work had not started; the collision result is recorded below.

## Phase 7 curated-corpus collision-export parity — accepted

- The locked selector produces exactly 30 unique floors: the six established historical sentinels plus eight accepted Phase 7 seeds per topology. The Phase 7 selections, in locked slot order, are processional `2026072308, 2026072500, 2026072484, 2026072310, 2026072942, 2026072304, 2026072300, 2026074216`; atrium `2026072661, 2026072357, 2026073589, 2026072309, 2026072857, 2026072301, 2026073273, 2026072397`; and twin-wing `2026072535, 2026072499, 2026073323, 2026072371, 2026073879, 2026072303, 2026072375, 2026073339`.
- Production and validation entry points share one `RandomDungeonSceneBuilder` rebuild core and the existing `GameplayCollisionExporter.ExportActiveSceneSharedCollisionData` implementation. Validation uses a unique temporary scene and data key, refuses the checked-in production destinations, and adds no alternate renderer or collision representation.
- All 30/30 floors pass actual export parity. Each enabled non-trigger rendered dungeon collider is a supported box or mesh and appears exactly once in both movement and query payloads; source/export counts match per floor across 6,660..8,532 colliders. The server and bundled client JSON copies are byte-identical independently for movement and query collision. There are no exporter errors, coded failures, or canonical-hash mismatches.
- The locked timing boundary includes production plan/render, generated-root resolution, spawn centering, collision-mesh normalization scan, collision marking, active-scene registration, and the actual file-writing shared-collision exporter. In the locked environment after 40 planning warm-ups and an unmeasured 30-floor export warm-up, mean is 1,911.718 ms/floor, p95 is 2,213.056 ms, and maximum is 3,048.965 ms. This passes the unchanged p95 2.5-second and maximum 5-second budgets with zero imports during measurement.
- The report separately retains the deliberately broader full-rebuild diagnostic, including temporary scene setup, camera/lighting, two scene serializations, and post-export asset saving: mean 3,725.558 ms, p95 4,252.547 ms, maximum 5,095.545 ms. These non-budget stages are not reclassified as plan/render/export. The earlier import-normalizing run and the over-broad saved-scene timing result remain preserved with explicit `_import_normalization_invalid` and `_saved_scene_boundary_overbroad` suffixes; neither replaced or weakened a locked threshold.
- The harness snapshots the synthesized-stair log plus every model-importer metadata file touched by warm-up, restores all 21 tracked artifacts byte-for-byte in `finally`, and deletes the unique temporary scene and four collision outputs. Cleanup passes, no temporary artifact remains, and the checked-in baked scene and production collision payloads are unchanged.
- The focused collision-support fixture passes 5/5 and all Dungeon Lab fixtures pass 83/83. `git diff --check` passes. The known full-suite baseline remains 362/383 with 21 unrelated failures and was not rerun. Active ignored evidence is `DungeonLabReports/phase7_collision_export_parity.json`; the final log and NUnit evidence are `Logs/phase7_collision_export_parity.log`, `Logs/phase7_collision_boundary_support_results.xml`, and `Logs/phase7_collision_final_all_dungeon_lab_results.xml`.
- The Phase 7 collision-export-parity gate now passes. At this evidence point no curated-gallery screenshot, randomized review order, score sheet, reviewer score, or human-review decision had been created; gallery generation is recorded below.

## Phase 7 blinded curated gallery — generated; human review skipped

- Diagnostic-only gallery support refuses to run unless the passing collision report matches the exact locked 30-floor corpus. It orders those floors by their already-locked canonical SHA-256 values, assigns only `F01..F30` reviewer IDs, and keeps seeds, topology names, historical categories, selection slots, canonical hashes, and automated metrics in a separately labeled internal manifest.
- The final review packet contains exactly 90 scored PNGs: one 1600x900 sentinel-framed overview, one 70-degree/1.65 m player-height arrival/threshold view through the resolved typed threshold port, and one player-height landmark/vista-approach view per floor. Floors with a rendered named promontory use the validated vista source-to-target line; other floors use the resolved landmark entry port. No annotation or route overlay is present.
- `review_packet/README.md` gives complete independent-review steps, the locked 1-5 scale, all seven per-floor criteria, reviewer qualifications, and the unchanged passing rule. Separate empty STEP 1 floor and STEP 2 anonymous-topology/overall repetition CSV templates exist for Reviewer 1 and Reviewer 2. No reviewer score or human acceptance decision has been entered.
- The artifact audit passes: 30 unique blinded floor IDs, three images each, 90/90 manifest SHA-256 matches, 90/90 exact 1600x900 dimensions, no forbidden identity/metric key in the review manifest, empty and matching reviewer templates, and 97 expected files total. Representative triplets across every anonymous topology group and all historical-control categories were visually inspected after correcting the framing.
...
- `TieredLevelPlan` now carries 1-4 canonical external connector promontories separately from the optional named-vista promontory. Each resolution records a stable cardinal identity, anchor, terminal port/cell, elevation, and ordered occupied cells. The approved hash-isolated count policy is 50% one, 31.25% two, 12.5% three, and 6.25% four, with at most one connector per cardinal direction.
- Resolution is the final plan mutation, consumes no shared random state, requires the whole exact-count set before adding any cell, places both appendage cells in flood-filled exterior void with a clear outward throat, excludes every protected structure/doorway/stair/recipe/scenic reservation, and rejects/retries with stable code `EXTERNAL_CONNECTOR_PROMONTORY` rather than weakening the requirement.
- Canonical/report versions advance to `dungeon-plan-v9` / `route-topologies-v9`. Exact pre-corrective projections prove all existing route, tier, stair, bridge, recipe, and named-vista structure remains unchanged. Rendering derives the two required open edges per connector, retains the shared promontory pier/abyss path, and removes only terminal-edge fascia so a later floor can attach without a wall or collision seal.
- Two independent locked-range runs over `2026072100..2026072299` each passed 200/200 accepted and hard-valid, exact 100/50/50 topology selection, p95/max layout attempts `1/1`, 200/200 exact desired counts and unique directions, the unchanged named-vista baseline `114 = 22/50/42`, and 200/200 exact pre-corrective structural matches. The observed connector distribution was `95/66/28/11` for counts `1/2/3/4`; both result hashes are `fac8e32570235a1ff3025cfc1c58269afb5bece46f51445da97ff8767551180d`.
- Run 1 planning performance was mean 77.496 ms, p95 104.976 ms, maximum 199.549 ms, and loop 15.503 seconds. Run 2 was mean 76.787 ms, p95 107.078 ms, maximum 130.645 ms, and loop 15.360 seconds. Both pass the approved 125/200/750 ms and 25-second thresholds in the locked environment.
- Exactly nine deterministic production floors passed the actual shared movement/query collision-export gate after the established unmeasured same-seed import warm-up: 9/9 source/export parity and unobstructed terminal floors, zero measured imports/errors, clean artifact restoration, and timing min/p50/p95/max `1,589.039/1,685.958/1,897.675/1,897.675 ms` under the approved 2.5/5-second limits.
- The focused stacked-crossing fixture uses the unchanged `AddAerialBridges`, `spanDeckLevels`, `TryValidateSpanHeadroom`, `ElevationEdgeModel`, and shared exporter paths. It proves one upper external-span bridge over a perpendicular independently traversable lower route, four-level headroom acceptance and forced two-level rejection, both rendered/colliding surfaces, open lower clearance, and exact movement/query export parity with no failure or cleanup code. No bridge frequency or production occurrence policy changed.
- Focused connector tests pass 4/4, stacked-crossing tests pass 3/3, and all Dungeon Lab EditMode tests pass 98/98. The full EditMode suite is 389/410 with exactly the same 21 unrelated baseline failures and no new failure name. Both editor and test assemblies compile; `git diff --check` passes.
- Active ignored evidence is `DungeonLabReports/corrective_connections_200_seed_comparison.json`, both `_corrective_runN` reports, `DungeonLabReports/corrective_nine_floor_collision_export.json`, and `DungeonLabReports/corrective_stacked_crossing_export.json`. No 2,000-seed sweep, gallery regeneration, or human review was run for this corrective item.
...
Completed this session: completed Slice 6 and the explicitly requested editor testing follow-up. One validated per-pattern count/eligibility policy is now the sole producer of `RouteIntent.plannedOverlooks`. Processional requests the two declared 4u/8u non-traversal seams; atrium and twin-wing request zero. The existing appendage and elevation-edge paths consume the result unchanged. Geometry evidence showed the former 12u threshold/branch-passage declaration never abutted, so it was removed with owner approval instead of retained as compatibility slop. Unity now exposes checked `Spacious` / `Dense` choices under `Arena > Dungeons > Generation Profile`; the per-user choice feeds the existing resolver, while the environment variable remains the command-line override and no-override batch mode remains reproducibly spacious.
Current validation result: all density/profile fixtures pass 32/32, including the persisted editor-choice and environment-override contract, and the editor assembly compiles with 0 errors. Spacious and dense smoke sweeps each passed twice at 50/50 accepted and hard-valid with identical settings digests, result hashes, and all per-seed canonical hashes. Spacious attempts are `1:50`; dense attempts are `1:44, 2:6`, with no validation failure or `STAIR_PLACEMENT` rejection. All twelve real-render captures across both profiles report `REJECTED 0`; the 4u and 8u processional contacts read as intentional cliff/railing seams.
