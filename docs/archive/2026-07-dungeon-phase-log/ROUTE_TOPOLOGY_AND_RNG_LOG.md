# Route topology, derived RNG and rebaseline log — July 2026

Archived 2026-07-29 from `docs/dungeon-builder/CURRENT_STATUS.md`, which had
regrown the per-phase evidence sections its own header forbids. **History, not
current constraints.** Every workstream below is closed.

Contents: the 2026-07-25 architecture-review landings, the derived-RNG
rebaseline, the variation regression traced to the route-first cutover and its
three steps (data cutover, rubber sheet, four new topologies), the post-rebaseline
measurements, and the `TryBuildCellLevelField` split.

Two things in here are still true and were carried forward into the live page
rather than left only in this archive:

- a recorded hash is a **transient comparison value**, never a gate — compare
  against the current commit with `ops/dungeon-port-ab.sh`;
- the two approaches measured wrong and reverted (blind pre-inflation stairwell
  shaft reservation, shrinking the lane pitch).

**One claim below is known wrong and is kept only because this is a record.**
The closing paragraph of the variation-regression section says bridges,
balconies and overlooks are "mostly a matter of *authoring* those edge kinds in
new graphs rather than building new systems", reasoning from
`RouteTransitionKind` already having a `Bridge` member. That is vocabulary, not
capability: bridges are capped at two per dungeon, rejected over room interiors,
and are not walkable surfaces at all. It misled a later design pass. See
[`layered-topology-design-2026-07-29.md`](../../dungeon-builder/layered-topology-design-2026-07-29.md)
§1.3 and §2.

Document links below were rewritten for this folder and resolve. Code paths are
written repo-root-relative, matching the convention in
`ARCHITECTURE_REVIEW_2026-07-25.md`.

---

### Landed 2026-07-25 (see [`ARCHITECTURE_REVIEW_2026-07-25.md`](../../dungeon-builder/ARCHITECTURE_REVIEW_2026-07-25.md) §12)
...
[`route-topology-authoring-2026-07-25.md`](../../dungeon-builder/route-topology-authoring-2026-07-25.md).
...
[`ROUTE_TOPOLOGY_AUTHORING.md`](../../dungeon-builder/ROUTE_TOPOLOGY_AUTHORING.md) for the format and
...
- **Recipe port geometry is an authoring rule that nothing checks.** The
  ...
  [`ROUTE_TOPOLOGY_AUTHORING.md`](../../dungeon-builder/ROUTE_TOPOLOGY_AUTHORING.md). Two of the four
