# Knockback — Formal Design (2026-07-17, rev 6)

Status: IMPLEMENTED — rev 2 resolved the five P1 contract gaps from the first review; rev 3 resolved the second review's findings; rev 4 records the owner's final knockback reaction contract; rev 5 increased the global knockback speed by 1.5x; rev 6 increases it to 24 m/s.

Rev 2 changes at a glance: knockback is now a **first-class effect packet** (not a `StatusApplication` masquerade) carrying impact-time direction; an ordering-independent guard keeps Stagger's micro-shove from replacing authored knockback; knockback **terminates self-initiated movement deliveries** (dash casts) instead of orphaning them; NPC AI is gated by the **forced-movement row itself**, not the hold stamp; the player mover's **non-scene-aware ground-Y sampling** is called out as a prerequisite fix; NPC resistance stays off the public catalog table; duration clamp contradiction removed. Rev 4 supersedes rev 2's narrower interruption policy.
...
Rev 4 changes at a glance: every effective knockback now applies the standard short **Stagger** reaction contract: interrupt casts/actions, apply 1 second of hard CC, and play the existing `hit_large`/stagger animation. Knockback and standalone Stagger use the same status/reaction rail; their gameplay difference is the displacement: knockback uses its authored distance and 12 m/s mover, while standalone Stagger retains its 0.45 m / 100 ms micro-shove. Shockwave therefore authors only `KNOCKBACK`, and the client reacts only to the resulting `STAGGER` status instead of also reacting to the movement row.

Rev 5 changes at a glance: `KNOCKBACK_SPEED_METERS_PER_SEC` increases from 12 m/s to 18 m/s. Authored distance, resistance, collision, CC duration, interruption, and reaction animation are unchanged.

Rev 6 changes at a glance: `KNOCKBACK_SPEED_METERS_PER_SEC` increases from 18 m/s to 24 m/s. Authored distance, resistance, collision, CC duration, interruption, and reaction animation remain unchanged.
...
- Every effective knockback interrupts casts/actions, applies the standard short Stagger CC, and plays the `hit_large`/stagger reaction.
...
Knockback is authored on the same `impact_effects` rail as statuses but is **not** a `StatusApplication`. Rationale (from review): `StatusApplication` cannot carry the impact-time direction that projectile and AoE knockbacks need. The first-class packet owns displacement; after resistance admits an effective shove, the resolver applies the ordinary short `STAGGER` status to reuse the established CC, interruption, and presentation contract.
...
- A successful `start_knockback_shove` applies a refreshing `STAGGER` status for `KNOCKBACK_STAGGER_DURATION_MS`. That status is the single owner of hard CC, cast/action interruption, and the `hit_large` reaction. It does not create the 0.45 m stagger shove because the knockback track already owns displacement.

Knockback is therefore a complete reaction, not a pure-displacement primitive. Abilities author only `KNOCKBACK`; they do not co-author `STAGGER` to obtain the baseline knockback behavior.
...
|---|---|---|
| `KNOCKBACK_SPEED_METERS_PER_SEC` | 24.0 | **Nominal** push speed; duration derived (`distance / speed`, floor one tick = 33 ms) |
| `KNOCKBACK_STAGGER_DURATION_MS` | 1000 | Short hard-CC/interruption window applied by every effective knockback |
...
Strength = authored distance, not an abstract tier. Duration is derived from the nominal speed with no independent clamp (the rev-1 duration clamp contradicted constant speed and is dropped). The one-tick duration floor means pushes under ~0.8 m travel slower than nominal — 24 m/s is the contract for every push above that, not a universal constant. After resistance scaling, duration shrinks with distance.
...
2. **Queue/resolve**: `EffectPacket::Knockback` → `PendingKnockback` (private table) → `resolve_pending_effects` arm → `start_knockback_shove(ctx, now, source, target, spell_id, dir, authored_distance)`. Zero-damage knockbacks are legal (no `requires_positive_damage` coupling). After resistance admits an effective shove, the resolver applies the standard short `STAGGER` hard CC without marking combat a second time. **Combat entry (review P1)**: damage marks harmful engagement only when the hit amount is positive (`combat.rs:3954`), so `PendingKnockback` resolution explicitly calls `mark_harmful_combat_action` after validating the hostile interaction.
3. **Shove (players)**: `start_knockback_shove` mirrors `start_stagger_shove` (`combat.rs:4778`) with parameters: applies resistance, derives duration from a per-shove **nominal speed**, bakes via `bake_linear_special_movement`, starts kind `KNOCKBACK` with `FACE_START` facing and `STOP_AT_BLOCK` collision. `start_stagger_shove` becomes a thin call into it — direction from `source_to_target_direction`, distance from `STAGGER_SHOVE_DISTANCE_METERS`, and **its own nominal speed derived from the authored `STAGGER_SHOVE_DISTANCE_METERS` / `STAGGER_SHOVE_DURATION_MS` pair (4.5 m/s)**, not the 24 m/s knockback rule. Both rules stay live; resistance scales distance and duration proportionally, preserving each shove's authored speed.
4. **Interruption & movement-ownership rules** (all flagged behavior changes):
   ...
   - **Single displacement owner.** `start_stagger_shove` **no-ops** when a `PendingKnockback` row exists for the target or an active externally imposed movement row already owns the target. This is required by knockback's own `STAGGER` side effect: the status supplies CC/interruption/presentation, while the authored knockback track remains the only displacement.
   - **Knockback interrupts all casts/actions.** Once resistance and the minimum-effective-distance check admit a shove, knockback starts its externally imposed movement track and applies `STAGGER`. The existing stagger side effects fizzle every active cast, clear queued melee followups, defense, movement action state, timed movement, and pending melee impacts while preserving the newly started external movement track. Immune or negligibly-pushed targets receive neither the status nor interruption.
...
5. **Shove (NPCs)** — the genuinely new machinery:
   ...
   - Client rendering: **ordinary `NpcPhysics` interpolation.** At 24 m/s the per-tick delta is ~0.8 m — under the 2.0 m hard-snap threshold, so `RemotePresentationBuffer` renders it as fast smooth motion, exactly how NPC chase movement renders today. No new public subscription and no handwritten client wiring for v1 (the new private table still triggers the standard binding regen, §3.6). (Extending `SpecialMovementRuntime` to NPC owners is the polish path if the smoothing reads poorly — deferred.)

### 3.6 Client changes

Player displacement plays through the existing `SpecialMovementRuntime` path on local and remote clients (sampler is kind-agnostic — verified); NPC displacement renders through normal physics interpolation; the resistance affix auto-appears on the character sheet. `PendingKnockback` and `NpcForcedMovement` are new tables and therefore required canonical binding regeneration and the schema-change publish path.

Reaction presentation:

- **Player reaction anim**: the inserted/refreshed `STAGGER` status routes through `EntityRegistry.TriggerStaggerPresentation` → `CombatStatusReactionController.TriggerStagger`, which selects the existing `hit_large` clips. `SpecialMovementRuntime` owns movement only and must not trigger a second reaction.
- **NPC reaction**: the same `STAGGER` status drives the NPC hard-CC/stagger reaction, including zero-damage knockbacks; ordinary HP-loss hit reactions remain separate.
...
| Situation | Ruling |
|---|---|
| Victim casting a non-movement spell | Cast is interrupted by knockback's short Stagger |
| Victim mid-dash / gap-close / dodge | Cast/action is interrupted and knockback takes over from the current position |
| Victim casting a movement-delivery spell | Cast is interrupted and its self-initiated movement state is cleared |
| Stagger + knockback on the same hit | Knockback owns displacement; one refreshing Stagger status owns CC/reaction |
...
| Resistance ≥ 1.0 (or effective distance < min) | No shove, no track, no Stagger/reaction, **no interruption** — the victim's action is untouched |
...
- `server/src/combat.rs` — `EffectPacket::Knockback`, queue/pending/resolve rail including combat entry, resisted shared mover, knockback-owned short `STAGGER` application, stagger no-op displacement guard, scoped runtime deletes, `StatusEffectKind::KnockbackResistance`, and temporary-modifier accumulation/accessor
...
**Client**
- `Assets/Arena/Runtime/Entity/EntityRegistry.cs` — `STAGGER` status is the sole player reaction dispatch; `SpecialMovementRuntime` only supplies movement
- `Assets/Arena/Runtime/Presentation/CombatStatusReactionController.cs` / `PlayerAnimator.cs` — existing `TriggerStagger` path selects the `hit_large` clips
...
1. **Phase 1 — server core (players)**: scene-aware mover Y fix (prerequisite), effect variant end-to-end (`ImpactEffect` enum, packet, pending table, resolve + combat-entry mark), `start_knockback_shove` + interruption/movement-ownership/teardown rules, resistance plumbing (equipment + buff kind + NPC template field parsed but unused), combat rules, and pilot spells. Deploy via the **schema-change publish path** + catalog re-sync (`publish_progression_catalogs`) + canonical binding regen — `ops/republish-catalog.sh` is explicitly for no-schema changes only (`ops/republish-catalog.sh:4`) and does not apply here.
...
3. **Phase 3 — presentation polish**: route knockback's standard Stagger status through the existing player/NPC stagger reactions; tooltip entry; optional hitstop/immune floater.

Verification (per phase, live): cast a pilot knockback spell at a fixture target; assert displacement distance/direction from `PlayerPhysics`/`NpcPhysics` deltas; repeat with resistance gear equipped (scaled) and an immune NPC (unmoved); shove-into-wall asserts the baked stop; casting and movement-action victims assert interruption plus a short `STAGGER` row; player/NPC presentation asserts one `hit_large` reaction; zero-damage push asserts the victim enters combat (regen stops); disconnect mid-push asserts no runtime row survives teardown. Catalog parse errors self-report at boot via the `deny_unknown_fields` panic.

## 7. Decision log

**Decided (owner, 2026-07-17 and 2026-07-18 reviews):**
...
- Every effective knockback applies the standard 1000 ms Stagger contract: interrupt all casts/actions, apply hard CC, and play `hit_large`. Abilities author only knockback distance; they do not compose a duplicate Stagger effect.
- Hostile zero-damage pushes count as harmful combat actions (combat entry, regen interruption).

**Still open:**
- Tuning defaults: 24 m/s push speed, 10 m distance cap, 0.6 gear cap — all in `combat_rules`, cheap to retune post-playtest.
