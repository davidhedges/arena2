...
use crate::progression::{
    action_id_is_selectable_loadout_action, active_selectable_ability_for_authored_action,
    movement_delivery_for_ability_id, movement_delivery_for_action_id,
    owner_has_active_selectable_action,
};
...
pub fn cast_request(
    ...
    if movement_delivery_for_action_id(spell_id.as_str()).is_some() {
        let Some(ability) =
            ...
            return Err(format!(
                "movement ability '{}' is not assigned on the active spec",
    ...
    if action_id_is_selectable_loadout_action(ctx, &authored_action_id)
        && !owner_has_active_selectable_action(ctx, ctx.sender(), &authored_action_id)
    {
        return Err(format!(
            "spell '{}' is not assigned on the active spec",
            kind.as_str()
        ));
    }
