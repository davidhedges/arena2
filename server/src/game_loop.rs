...
use crate::world_interactions::tick_world_interactions;
use crate::world_obstacles::{expire_world_obstacles, resolve_active_world_obstacle_movement};
use crate::world_traps::{collect_world_trap_tick_players, expire_world_traps, tick_world_traps};
...
const PRE_TICK_SUBPHASE_COUNT: usize = 18;
...
const PRE_SUB_WORLD_INTERACTIONS: usize = 7;
const PRE_SUB_MELEE_FOLLOWUPS: usize = 8;
const PRE_SUB_AUTO_ATTACKS: usize = 9;
const PRE_SUB_NPC_COMBAT: usize = 10;
const PRE_SUB_COMBAT_CYCLE: usize = 11;
const PRE_SUB_SPELL_PROJECTILE_SIM: usize = 12;
const PRE_SUB_PERIODIC_EFFECTS: usize = 13;
const PRE_SUB_PASSIVES_AURAS: usize = 14;
const PRE_SUB_EXPIRIES: usize = 15;
const PRE_SUB_MOVEMENT_MODIFIERS: usize = 16;
// Appended rather than slotted next to `world_interactions` so the existing
// indices — and every profile report keyed off them — stay put.
const PRE_SUB_WORLD_TRAPS: usize = 17;
...
fn run_pre_tick_housekeeping_phase(
    ...
    timed_subphase(
        profiling,
        stopwatch_active,
        PRE_TICK_SUBPHASE_NAMES[PRE_SUB_WORLD_INTERACTIONS],
        &mut subphase_micros[PRE_SUB_WORLD_INTERACTIONS],
        || tick_world_interactions(ctx, now),
    );
    ...
    // Traps share this scope so they reuse the already-built snapshot set and
    // its spatial index. `trap_players` is the whole-phase early out: an empty
    // dungeon builds no snapshot set on the traps' account.
    let trap_players = collect_world_trap_tick_players(ctx);
    let mut world_trap_micros = 0u32;
    timed_subphase(
        profiling,
        stopwatch_active,
        ...
        || -> Result<(), String> {
            ...
            if !has_active_bespoke_spells
                && !has_active_combat_projectiles
                && trap_players.is_empty()
            {
                return Ok(());
            }
            ...
            timed_subphase(
                profiling,
                stopwatch_active,
                PRE_TICK_SUBPHASE_NAMES[PRE_SUB_WORLD_TRAPS],
                &mut world_trap_micros,
                || tick_world_traps(ctx, now, &actor_snapshots, &trap_players),
            );
            Ok(())
        },
    )?;
    subphase_micros[PRE_SUB_WORLD_TRAPS] =
        subphase_micros[PRE_SUB_WORLD_TRAPS].saturating_add(world_trap_micros);
...
fn tick_special_movement_runtimes(ctx: &ReducerContext, now: Timestamp) {
    ...
    for runtime in runtimes {
        ...
        let (desired_x, desired_y, desired_z, desired_yaw, mut finished) =
        ...
        if finished {
            ...
            physics.vel_x = (resolved_x - prev_x) / FIXED_TICK_SECONDS;
...
mod tests {
    ...
    fn sample(total_micros: u32, pre_micros: u32) -> TickProfileSample {
        TickProfileSample {
            ...
            pre_subphase_micros: [pre_micros / super::PRE_TICK_SUBPHASE_COUNT as u32;
                super::PRE_TICK_SUBPHASE_COUNT],
        }
    }
